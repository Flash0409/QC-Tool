...
class CircuitInspector:
    ...
    # ============================================================================
    # UPDATED export_annotated_pdf METHOD
    # This replaces the export section in your code to properly handle highlighters
    # ============================================================================
                        ...
                        if len(stroke) >= 2:
                            ...
                            if color_key == 'orange' and 'bbox_page' in ann:
                                ...
                                if sr_text:
                                    ...
                                    text_pos = fitz.Point(bbox_rect.x0, max(bbox_rect.y0 - 12, bbox_rect.y0))
                                    try:
                                        target_page.insert_text(text_pos, sr_text, fontsize=8, color=(1, 0, 0))
                                    except:
                                        pass

                # -------- OLD RECTANGLE ANNOTATIONS (OK/ERROR) --------
                elif ann_type in ('ok', 'error') and 'bbox_page' in ann:
                    x1, y1, x2, y2 = ann['bbox_page']
    ...
    def sync_manager_stats_only(self):
        ...
        try:
            ...
            # Update ONLY the statistics columns in database (preserves status)
            conn = sqlite3.connect(self.manager_db.db_path)
            cursor = conn.cursor()
